# 文件名：eval_internvl3_text_retrieval.py
# 作用：用 InternVL3-8B 做“纯文本/图谱片段检索”
...
MODEL_PATH = '/home/workspace/jxk/sourcecode/InternVL-all/InternVL3/InternVL3-8B'
...
# ===============================================
# InternVL3 作为“文本 reranker”的调用
# ===============================================

def internvl_chat_text(model, tokenizer, prompt: str) -> str:
    """
    用 InternVL3 的 chat 接口做纯文本对话。
    不传 image，不改你的 prompt 逻辑。
    """
    # InternVL3 的 chat 接口支持 pixel_values=None 用纯文本
...
def evaluate_with_internvl(documents, queries, model, tokenizer, mode='text'):
    ...
    for q_data in tqdm(queries, desc=f"Eval InternVL3 ({mode.upper()})", leave=False):
...
if __name__ == "__main__":
    print("📥 Loading InternVL3-8B Model (for text retrieval)...")
